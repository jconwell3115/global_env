...
| Term | Description |
|------|-------------|
| **Vault** | A folder on your device that contains all your notes (Markdown files). |
| **Note** | A single Markdown file (`.md`) inside your Vault. |
| **Link** | Connects notes using `[[Note Name]]`. |
| **Backlink** | A note that links _to_ the current note. |
| **Graph View** | Visual representation of how your notes are connected. |
| **Tag** | Metadata using `#tagname` for categorization. |
| **YAML Frontmatter** | Metadata block at the top of a note using `---`. |
...
| Code Block      | `lang<br>code<br>`<br> | `js<br>console.log("Hi")<br>` |
| Bullet List     | `-`, `*`, `+`          | `- Item`                      |
| Numbered List   | `1.`                   | `1. First`                    |
| Checkbox        | `- [ ]` / `- [x]`      | `- [x] Done`                  |
| Link to Note    | `[[Note Name]]`        | `[[My Note]]`                 |
| Embed Note      | `![[Note Name]]`       | Embeds full note              |
| External Link   | `text`                 | Google                        |
| Image           | `!`                    | `!`                           |
| Block Reference | `^block-id`            | Refer to specific block       |
...
| ------------- | -------------------- | ---------------------- |
| Internal Link | `[[Note Name]]`      | Links to another note  |
| Aliases       | `[[Note NameAlias]]` | Display different text |
| Header Link   | `[[Note#Header]]`    | Link to a section      |
| Block Link    | `[[Note^block-id]]`  | Link to a block        |
| Tag           | `#tag`               | Add metadata           |
| Tag Pane      | `Ctrl+Shift+T`       | View all tags          |
...
| `path:` | `path:"folder/"` | Search in folder |
| `file:` | `file:NoteName` | Search in file |
| `-"term"` | `-draft` | Exclude term |
| `line:(text)` | `line:(meeting)` | Search in lines |
